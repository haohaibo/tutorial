...
"        for(int i = 0; i < WPT; ++i){\n"
"           Asub[row][col*WPT + i] = A[(t*TS + col*WPT + i)*M + get_group_id(0)*TS + row];\n"
"           Bsub[row][col*WPT + i] = B[(get_group_id(1)*TS + col*WPT + i)*K + (t*TS + row)];\n"
"        }\n"
...
"    for (int w = 0; w < WPT; ++w){\n"
"        for (int k = 0; k < TS; ++k){\n"
"            acc[w] += Asub[row][k] * Bsub[k][col*WPT + w];\n"
"        }\n"
"    }\n"
...
"    }\n"
"   for (int w = 0; w < WPT; ++w){\n"
"       C[(get_group_id(1)*TS + col*WPT + w)*M + get_group_id(0)*TS + row] = acc[w];\n"
"   }\n"
"}\n";
