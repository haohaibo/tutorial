...
__kernel void gemm(
    ...
    for(int i = 0; i < K/NUM_UNROLL; ++i){
        ...
        tileB[local_thread_id + 0*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 0];
        tileB[local_thread_id + 1*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 1];
        tileB[local_thread_id + 2*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 2];
        tileB[local_thread_id + 3*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 3];
        tileB[local_thread_id + 4*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 4];
        tileB[local_thread_id + 5*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 5];
        tileB[local_thread_id + 6*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 6];
        tileB[local_thread_id + 7*tile_dim_y] = 
            B[(block_id_y*tile_dim_y + local_thread_id)*K + i*NUM_UNROLL + 7];
        ...
        if(i % 2 == 0){
            // register -> shared
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 0] = loadA[0];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 1] = loadA[1];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 2] = loadA[2];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 3] = loadA[3];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 4] = loadA[4];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 5] = loadA[5];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 6] = loadA[6];
            tileA[local_thread_id_y*64 + local_thread_id_x*8 + 7] = loadA[7];

            tileB[local_thread_id + 0*tile_dim_y] = loadB[0];
            tileB[local_thread_id + 1*tile_dim_y] = loadB[1];
            tileB[local_thread_id + 2*tile_dim_y] = loadB[2];
            tileB[local_thread_id + 3*tile_dim_y] = loadB[3];
            tileB[local_thread_id + 4*tile_dim_y] = loadB[4];
            tileB[local_thread_id + 5*tile_dim_y] = loadB[5];
            tileB[local_thread_id + 6*tile_dim_y] = loadB[6];
            tileB[local_thread_id + 7*tile_dim_y] = loadB[7];
            ...
            // register -> shared
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 0] = loadA[0];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 1] = loadA[1];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 2] = loadA[2];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 3] = loadA[3];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 4] = loadA[4];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 5] = loadA[5];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 6] = loadA[6];
            tileA_bak[local_thread_id_y*64 + local_thread_id_x*8 + 7] = loadA[7];

            tileB_bak[local_thread_id + 0*tile_dim_y] = loadB[0];
            tileB_bak[local_thread_id + 1*tile_dim_y] = loadB[1];
            tileB_bak[local_thread_id + 2*tile_dim_y] = loadB[2];
            tileB_bak[local_thread_id + 3*tile_dim_y] = loadB[3];
            tileB_bak[local_thread_id + 4*tile_dim_y] = loadB[4];
            tileB_bak[local_thread_id + 5*tile_dim_y] = loadB[5];
            tileB_bak[local_thread_id + 6*tile_dim_y] = loadB[6];
            tileB_bak[local_thread_id + 7*tile_dim_y] = loadB[7];
