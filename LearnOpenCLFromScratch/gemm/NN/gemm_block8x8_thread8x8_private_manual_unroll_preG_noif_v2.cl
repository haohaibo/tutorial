...
__kernel void gemm(
    ...
    for(int i = 0; i < K/NUM_UNROLL; ++i){
            ...
            for(int k = 0; k < NUM_UNROLL; ++k){
                ...
                regA[0] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 0];
                regA[1] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 1];
                regA[2] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 2];
                regA[3] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 3];
                regA[4] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 4];
                regA[5] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 5];
                regA[6] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 6];
                regA[7] = tileA[swap_index + k*tile_dim_x + local_thread_id_x*8 + 7];


                regB[0] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 0];
                regB[1] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 1];
                regB[2] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 2];
                regB[3] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 3];
                regB[4] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 4];
                regB[5] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 5];
                regB[6] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 6];
                regB[7] = tileB[swap_index + k*tile_dim_y + local_thread_id_y*8 + 7];
