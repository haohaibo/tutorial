...
float * matrixTranspose(int M, int N, float* A){
    float* A_trans = malloc(N*M*sizeof(float));
    for(int i = 0; i < N; ++i){
        for(int j = 0; j < M; ++j){
            A_trans[i*N + j] = A[j*M + i];
        }
    }
    return A_trans;
}
...
int main(int argc, char* argv[]){
    ...
    cl_program clCreateProgramWithSource (
        ...
        //char * gemm_kernel = getKernelSource("gemm_block8x8_thread8x8.cl");
        //char * gemm_kernel = getKernelSource("gemm_block8x8_thread8x8_private.cl");
    ...
    for(int i = 0; i < M; ++i)
    {
        for(int j = 0; j < N; ++j){
            temp = 0;
            for(int k = 0; k < K; ++k){
                //temp += h_a[k*M + i]*h_b[k*K + j]; 
                temp += h_a[k*M + i]*h_b[k*N + j]; 
                //printf("h_b[%d] = %f", j*K + k, h_b[j*K + k]);
            }
            //h_c[j*M + i] = temp;
            //printf("%f ",temp);
#if 1 
