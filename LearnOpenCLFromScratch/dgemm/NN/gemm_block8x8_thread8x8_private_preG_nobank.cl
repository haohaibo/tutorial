    ...
    for(int i = 0; i < K/NUM_UNROLL; ++i){
        ...
        if(i % 2 == 0){
            ...
            for(int k = 0; k < NUM_UNROLL; ++k){
                ...
                // load A, B from shared memory to registers and compute C
                regA[0] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 0];
                regA[1] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 1];
                regA[2] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 2];
                regA[3] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 3];
#endif
                // load A, B from shared memory to registers and compute C
                regA[0] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 0];
                regA[1] = tileA_bak[k*tile_dim_x + local_thread_id_x*4 + 1];
                ...
                regA[4] = tileA_bak[k*tile_dim_x + tile_dim_x/2 + local_thread_id_x*4 + 0];
                regA[5] = tileA_bak[k*tile_dim_x + tile_dim_x/2 + local_thread_id_x*4 + 1];
                regA[6] = tileA_bak[k*tile_dim_x + tile_dim_x/2 + local_thread_id_x*4 + 2];
                regA[7] = tileA_bak[k*tile_dim_x + tile_dim_x/2 + local_thread_id_x*4 + 3];


                regB[0] = tileB_bak[k*tile_dim_y + local_thread_id_y*4 + 0];
                regB[1] = tileB_bak[k*tile_dim_y + local_thread_id_y*4 + 1];
                regB[2] = tileB_bak[k*tile_dim_y + local_thread_id_y*4 + 2];
                regB[3] = tileB_bak[k*tile_dim_y + local_thread_id_y*4 + 3];

                regB[4] = tileB_bak[k*tile_dim_y + tile_dim_y/2 + local_thread_id_y*4 + 0];
                regB[5] = tileB_bak[k*tile_dim_y + tile_dim_y/2 + local_thread_id_y*4 + 1];
                regB[6] = tileB_bak[k*tile_dim_y + tile_dim_y/2 + local_thread_id_y*4 + 2];
                regB[7] = tileB_bak[k*tile_dim_y + tile_dim_y/2 + local_thread_id_y*4 + 3];
