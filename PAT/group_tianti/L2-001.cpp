...
int max_num_path(int src, int dst, int& max_people, int temp_people,struct PRE *Pre , int *people, std::vector<int>& max_path, std::vector<int> temp_path)
{
    ...
    {
        sum += max_num_path(src, Pre[dst].pre_node[i], max_people, temp_people + people[dst], Pre, people, max_path, temp_path);
    }
    return sum; 
}
int main()
{
    ...
    temp_path.reserve(N);
    //int max_num_path(int src, int dst, int& max_people, int temp_people,struct PRE *Pre , int *people, std::vector<int>& max_path, std::vector<int> temp_path)
    num_path = max_num_path(S, D, max_people, 0, Pre ,people, max_path, temp_path);
    printf("%d %d\n", num_path, max_people);
