...
class Solution {
    public:
        ...
        bool binary_search(vector< vector<int> >& matrix, int target, int low, int high, int m , int n)
        {
            if(low <= high)
            {
                ...
                {
                 vector<int>::iterator it = find(matrix[middle].begin(), matrix[middle].end(), target);
